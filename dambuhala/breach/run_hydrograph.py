...
Outputs
-------
<output_dir>/hydrograph.csv   — time (hours) and discharge (m³/s) columns
...
All units SI: Vw in m³, Qp in m³/s, dt in hours.
"""
...
def save_csv(t, q, path):
    header = "time_hours,discharge_m3s"
    data = np.column_stack([t, q])
...
def save_plot(t, q, a, b, path):
    ...
    ax.plot(t, q, color="steelblue", linewidth=1.5)
    ax.set_xlabel("Time (hours)")
    ax.set_ylabel("Discharge (m³/s)")
    ax.set_title(fr"Breach hydrograph  —  $\beta$: $a={a:.2f},\; b={b:.2f}$")
    ax.set_xlim(left=0)
...
def main():
    ...
    parser.add_argument("--Vw",  type=float, required=True,
                        help="Total breach outflow volume (m³)")
    parser.add_argument("--Qp",  type=float, required=True,
                        help="Peak discharge (m³/s)")
    ...
    print(f"Hydrograph generated: a={chosen_a}, b={chosen_b}, "
          f"T={h.T:.2f} h, Qp={np.max(q):.1f} m³/s")
